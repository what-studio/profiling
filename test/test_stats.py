...
from profiling.sortkeys import by_name, by_own_calls, by_all_time_per_call
...
def test_stat():
    ...
    assert stat.all_time_per_call == 0
    stat.all_time = 128
    stat.own_calls = 4
    assert stat.all_time_per_call == 32
    assert len(stat) == 0
...
def test_sorting():
    ...
    assert stat.sorted(by_all_time_per_call) == [stat2, stat3, stat1]
